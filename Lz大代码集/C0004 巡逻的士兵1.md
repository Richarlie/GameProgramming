...
unsigned int calculate(unsigned int n) {
    if (n <= 3) {
        return 0; // 如果士兵数小于等于3，所有士兵都会被选中
    }
    if (memo.find(n) != memo.end()) {
        return memo[n]; // 如果已经计算过，直接返回结果
    }

    // 如果士兵数是偶数，去掉偶数位置后，问题规模减半
    if (n % 2 == 0) {
        memo[n] = calculate(n / 2);
    }
    // 如果士兵数是奇数，去掉奇数位置后，问题规模变为((n + 1) / 2)
    else {
        memo[n] = calculate((n + 1) / 2);
    }

    // 如果去掉偶数位置后，返回的结果是0，说明当前最小编号位置是2
    if (memo[n] == 0) {
        memo[n] = 2;
    }
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTYzNTM4MjgzNCwtMTQ0ODY5OTExNSwtNj
U5NzQ3NDMzLDE2NzU1NTMxLC0xODc3OTIzNTE2LC0xNDUyMDg1
NjE2LDg4NTE2MzcyLDU5MDA4MzU2Nl19
-->
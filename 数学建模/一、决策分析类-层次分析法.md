# 一、决策分析类-层次分析法
**需求**：根据“流量 质量 颜值”等指标去评价一个明星的价值
可以对指标按照其重要性赋予权重，但问题是权重的赋予通常过于主观

![输入图片说明](/imgs/2025-04-29/8Nr0HwmWY5k1cWLR.png)
**如何客观设置权重？**此时层次分析法就起作用了
`归化处理`+`权重处理`
`归化处理`：把每个指标变成0~1之间的数值


**基本步骤**：
1. 建立模型
2. 构造判断矩阵
3. 一致性检验
4. 求权重后评价

- **第一步**：建立模型
建立递阶层次结构模型
目的：明确各指标与对象的关系
![输入图片说明](/imgs/2025-04-29/xacKbcbSFuu63lEh.png)
![输入图片说明](/imgs/2025-04-29/oqrkpNTK11futxpv.png)

- **第二步**：构造判断矩阵
- **目的**：对各指标的重要性进行两两比较，以科学得到权重
- 矩阵元素 $a_{ij}$：第i个指标相对第j个指标的重要程度
- **比较标准**：
![输入图片说明](/imgs/2025-04-29/sgw7bDSOwZMLiovw.png)

然而，如果指标过多时，两两比较后难免会造成整体的有失偏颇（也就是指标的重要程度不一致）
此时便需要一致性检验了

- **第三步**：一致性检验
**正互反矩阵**：满足$a_{ij}>0且a_{ij}\times a_{ji}=1$的矩阵
$a_{ij}=a_{ik}\times a_{kj}$
层次分析法中构造的判断矩阵都是正互反矩阵
**一致矩阵**：满足$a_{ij}\times a_{ik}=a_{kj}$的正互反矩阵
或是满足$a_{ij}\times a_{ik}=a_{kj}$且其各行（列）成倍数关系的矩阵
**一致性检验原理**就是：检验构造的判断矩阵与一致矩阵的差别是否过大
- 步骤：
- 1. 计算一致性指标CI：
$CI=\frac{\lambda_{max}-n}{n-1}$（$\lambda_{max}$是最大特征值）
（其中，n代表阶数——指标数量）
- 2. 查找平均随机一致性指标RI：
![输入图片说明](/imgs/2025-05-02/yHUUuhYP7glSZc1Y.png)
- 3. 计算一致性比例CR：
$CR=\frac{CI}{RI}$
$CR=0$：一致矩阵
$CR<0.1$矩阵一致
$CR\geq0.1$矩阵不一致
**若特征值中有虚数，则比较的是特征值的模长** 

- **第四步**：求权重
算术平均法
			![输入图片说明](/imgs/2025-05-02/xHrd9RCWMJcgKkF1.png)
  **例子**：先是竖向归一（$\frac{此项}{所有项之和}$），后是横向相加再归一（$\frac{此行之和}{所有行之和}$）
![输入图片说明](/imgs/2025-07-30/9SOeiooSiPK23Nsd.png)

几何平均法
![输入图片说明](/imgs/2025-05-02/CZUsH8yE2g7iaBi8.png)
 **例子**：先是横向相乘开方，后是竖向归一（$\frac{此行}{所有行之和}$）
 ![输入图片说明](/imgs/2025-07-30/gqcmReLbw69GYqba.png)

特征值法：
  ![输入图片说明](/imgs/2025-07-30/kDXAGiNxnzAEnjZo.png)
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTI2MTQwNzM1Miw5NTA1OTE5NjIsMTYwNj
AzNzE3NywtMjExMzI1ODYwMiwtMjAxMzA0NDk2OCw0NzE3ODA4
NzAsMTQwNTU1NTk5LDE5MjMxMDI4NDcsMjEzOTQzMjM2NSwxMz
k3ODcyODEzLC0xODE1NTMyMjEzLDM3OTI4NzE2NywtNzE5NTY5
MDU2LC04MDk4MzI0NzcsMTI3MjMwMzA0LDEwMDYyMzQxNjUsNT
M0ODEzMDQ4LDY1OTc1NzYsMTcxMzg4OTk3MCwzOTg1NTcwNF19

-->